...
class ModelExtensionMazaCatalogProduct extends Model
{
	...
	public function getProducts(array $data = array()): array
	{
		...
		if (isset($data['filter_special']) || !empty($data['filter_min_special_perc']) || !empty($data['filter_rating']) || !empty($data['filter_min_rating']) || !empty($data['filter_max_rating']) || !empty($data['filter_min_price']) || !empty($data['filter_max_price'])) {
			...
			$sql = 'SELECT product_id FROM ' . DB_PREFIX . 'mz_product_result';
		} else {
			...
			if (in_array('p.price', $sort)) {
				$sql .= ", (SELECT price FROM " . DB_PREFIX . "product_discount WHERE product_id = p.product_id AND customer_group_id = '" . (int)$this->config->get('config_customer_group_id') . "' AND quantity = '1' AND ((date_start = '0000-00-00' OR date_start < NOW()) AND (date_end = '0000-00-00' OR date_end > NOW())) ORDER BY priority DESC, price DESSC LIMIT 1) AS discount";
				$sql .= ", (SELECT price FROM " . DB_PREFIX . "product_special ps WHERE product_id = p.product_id AND customer_group_id = '" . (int)$this->config->get('config_customer_group_id') . "' AND ((date_start = '0000-00-00 00:00:00' OR date_start < NOW()) AND (date_end = '0000-00-00 00:00:00' OR date_end > NOW())) AND (mz_quantity = 0 OR mz_quantity > IFNULL((SELECT SUM(op.quantity) FROM " . DB_PREFIX . "order_product op LEFT JOIN `" . DB_PREFIX . "order` o ON op.order_id = o.order_id WHERE op.product_id = ps.product_id AND o.store_id = '" . (int)$this->config->get('config_store_id') . "' AND o.customer_group_id = '" . (int)$this->config->get('config_customer_group_id') . "'" . ($this->mz_skin_config->get('catalog_sold_order_statuses') ? " AND o.order_status_id IN (" . implode(',', array_map('intval', $this->mz_skin_config->get('catalog_sold_order_statuses'))) . ")" : "") . " AND (o.date_added > ps.date_start OR ps.date_start = '0000-00-00 00:00:00') AND (o.date_added < ps.date_end OR ps.date_end = '0000-00-00 00:00:00') GROUP BY op.product_id),0)) ORDER BY ps.priority DESC, ps.price DESC LIMIT 1) AS special";
			}
		...
		if ($product_table == DB_PREFIX . 'product') {
			...
			if (in_array('price', $additional_field) || !empty($data['filter_min_price']) || !empty($data['filter_max_price']) || isset($data['filter_special'])) {
				if ($this->config->get('config_tax')) {
					...
					$sql .= ", (SELECT price + IFNULL(ftax.total,0) + ((IFNULL(ptax.total,0) * price) / 100) FROM " . DB_PREFIX . "product_special ps WHERE product_id = p.product_id AND customer_group_id = '" . (int)$this->config->get('config_customer_group_id') . "' AND ((date_start = '0000-00-00 00:00:00' OR date_start < NOW()) AND (date_end = '0000-00-00 00:00:00' OR date_end > NOW())) AND (mz_quantity = 0 OR mz_quantity > IFNULL((SELECT COALESCE(SUM(op.quantity), 0) FROM " . DB_PREFIX . "order_product op LEFT JOIN `" . DB_PREFIX . "order` o ON op.order_id = o.order_id WHERE op.product_id = ps.product_id AND o.store_id = '" . (int)$this->config->get('config_store_id') . "' AND o.customer_group_id = '" . (int)$this->config->get('config_customer_group_id') . "'" . ($this->mz_skin_config->get('catalog_sold_order_statuses') ? " AND o.order_status_id IN (" . implode(',', array_map('intval', $this->mz_skin_config->get('catalog_sold_order_statuses'))) . ")" : "") . " AND (o.date_added > ps.date_start OR ps.date_start = '0000-00-00 00:00:00') AND (o.date_added < ps.date_end OR ps.date_end = '0000-00-00 00:00:00') GROUP BY op.product_id),0)) ORDER BY priority ASC, price ASC LIMIT 1) AS special";
				} else {
					$sql .= ", p.price";
			...
			if (in_array('special_perc', $additional_field) || !empty($data['filter_min_special_perc'])) {
				$sql .= ", (SELECT 100 - ((price * 100) / p.price) FROM " . DB_PREFIX . "product_special ps WHERE product_id = p.product_id AND customer_group_id = '" . (int)$this->config->get('config_customer_group_id') . "' AND ((date_start = '0000-00-00 00:00:00' OR date_start < NOW()) AND (date_end = '0000-00-00 00:00:00' OR date_end > NOW())) AND (mz_quantity = 0 OR mz_quantity > IFNULL((SELECT COALESCE(SUM(op.quantity), 0) FROM " . DB_PREFIX . "order_product op LEFT JOIN `" . DB_PREFIX . "order` o ON op.order_id = o.order_id WHERE op.product_id = ps.product_id AND o.store_id = '" . (int)$this->config->get('config_store_id') . "' AND o.customer_group_id = '" . (int)$this->config->get('config_customer_group_id') . "'" . ($this->mz_skin_config->get('catalog_sold_order_statuses') ? " AND o.order_status_id IN (" . implode(',', array_map('intval', $this->mz_skin_config->get('catalog_sold_order_statuses'))) . ")" : "") . " AND (o.date_added > ps.date_start OR ps.date_start = '0000-00-00 00:00:00') AND (o.date_added < ps.date_end OR ps.date_end = '0000-00-00 00:00:00') GROUP BY op.product_id),0)) ORDER BY priority ASC, price ASC LIMIT 1) AS special_perc";
			}
		}
	...
	{
		$query = $this->db->query("SELECT url FROM " . DB_PREFIX . "mz_product_video WHERE product_id = '" . (int)$product_id . "' ORDER BY sort_order ASC");

		return array_column($query->rows, 'url');
	}
