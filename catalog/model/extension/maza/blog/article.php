...
class ModelExtensionMazaBlogArticle extends Model {
	...
	public function getArticles(array $data = array()): array {
		...
		$sql .= " LEFT JOIN " . DB_PREFIX . "mz_blog_article_to_store a2s ON (a.article_id = a2s.article_id) LEFT JOIN " . DB_PREFIX . "mz_blog_author aa ON (aa.author_id = a.author_id) WHERE a.status = '1' AND a.date_available <= NOW() AND aa.status = '1' AND a2s.store_id = '" . (int)$this->config->get('config_store_id') . "'";
		...
		if(isset($data['sort_order'])){
			...
			if (isset($data['order']) && ($data['order'] == 'DESC')) {
					$sql .= " DESC, a.sort_order DESC";
			} else {
					$sql .= " ASC, a.sort_order ASC";
			}
		} else {
