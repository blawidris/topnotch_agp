...
class ControllerAccountCustomerpartnerAddproduct extends Controller {
	...
	protected function htmlfilters()
	{
			...
			foreach ($this->request->post['product_description'] as $language_id => $value) {
				...
				$this->request->post['product_description'][$language_id]['meta_title'] = $this->model_customerpartner_htmlfilter->HTMLFilter(html_entity_decode($value['name']),'');
				$this->request->post['product_description'][$language_id]['meta_description'] = $this->model_customerpartner_htmlfilter->HTMLFilter(html_entity_decode($value['name']),'');
				$this->request->post['product_description'][$language_id]['meta_keyword'] = $this->model_customerpartner_htmlfilter->HTMLFilter(html_entity_decode($value['tag']),'');
...
// 			}
		...
		if (isset($value['model']) &&  is_string($value['model']) && (utf8_strlen($this->request->post['model']) < 3) || (utf8_strlen($this->request->post['model']) > 64)) {
		...
		if($this->config->get('marketplace_noofimages')){
			$image = $this->config->get('marketplace_noofimages');
		}else{
			$image = 0;
		}
			$this->error['warning'] = $this->language->get('error_no_of_images').$image;
		}
		if(isset($this->request->post['product_special_tab']) && $this->request->post['product_special_tab']){
			$date_limit = strtotime(date("Y-m-d"));
			if(isset($this->request->post['product_special']) && $this->request->post['product_special']){
