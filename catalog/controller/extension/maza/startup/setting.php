...
class ControllerExtensionMazaStartupSetting extends Controller {
	public function index(): Action {
        ...
        if($layout_info){
            ...
            if(!empty($layout_info['mz_override_skin']['skin'][$this->config->get('config_store_id')])){
                $this->config->set('mz_layout_skin_id', $layout_info['mz_override_skin']['skin'][$this->config->get('config_store_id')]);
            }
        }
        ...
        // Skin
        $skin_id = $this->mz_theme_config->get('skin_id');
        
        // Load skin setting
        if(isset($this->request->get['mz_skin_id']) && (isset($this->session->data['user_id']) || $this->config->get('maza_developer_mode'))){
            $skin_id = (int)$this->request->get['mz_skin_id'];
        } elseif($this->config->get('mz_layout_skin_id')){
            $skin_id = $this->config->get('mz_layout_skin_id');
        }
        
        $skin_info = $this->model_extension_maza_skin->getSkin($skin_id);
        
        if(empty($skin_info) && $this->mz_theme_config->get('skin_id') != $skin_id){ // Use active skin of theme
            $skin_info = $this->model_extension_maza_skin->getSkin($this->mz_theme_config->get('skin_id'));
        } else {
            $this->mz_theme_config->set('skin_id', $skin_id);
        }
        ...
        if(version_compare(VERSION, '3.0.0.0') < 0){
            $this->config->set($this->config->get('config_theme') . '_image_category_width', $this->mz_skin_config->get('catalog_category_image_width'));
            $this->config->set($this->config->get('config_theme') . '_image_category_height', $this->mz_skin_config->get('catalog_category_image_height'));
            $this->config->set($this->config->get('config_theme') . '_image_thumb_width', $this->mz_skin_config->get('catalog_thumb_image_width'));
            $this->config->set($this->config->get('config_theme') . '_image_thumb_height', $this->mz_skin_config->get('catalog_thumb_image_height'));
            $this->config->set($this->config->get('config_theme') . '_image_additional_width', $this->mz_skin_config->get('catalog_thumb_image_width') / $this->mz_skin_config->get('catalog_additional_image_slides'));
            $this->config->set($this->config->get('config_theme') . '_image_additional_height', $this->mz_skin_config->get('catalog_thumb_image_height') / $this->mz_skin_config->get('catalog_additional_image_slides'));
        }
        
        return new Action('extension/maza/startup/event');
	}
}
