...
class ControllerExtensionMazaStartupSeoUrl extends Controller {
	public function index(): Action {
		...
		return new Action('extension/maza/startup/startup');
	}
	...
	public function rewrite(string $link): string {
		...
		// if (isset($data['route'])) {
		// 	if (strpos($data['route'], 'extension/maza/blog') === 0) {
		// 		$url .= '/blog';
		// 	}
		// }
		...
		foreach ($data as $key => $value) {
			if (isset($data['route'])) {
				if (($data['route'] == 'extension/maza/blog/article' && $key == 'article_id') || (($data['route'] == 'extension/maza/blog/author' || $data['route'] == 'extension/maza/blog/article') && $key == 'author_id')) {
					if(version_compare(VERSION, '3.0.0.0') < 0){ // For opencart 2
						$query = $this->db->query("SELECT * FROM " . DB_PREFIX . "url_alias WHERE `query` = '" . $this->db->escape('mz_blog_' . $key . '=' . (int)$value) . "'");
					} else {
					...
					if(version_compare(VERSION, '3.0.0.0') < 0){ // For opencart 2
						$query = $this->db->query("SELECT * FROM " . DB_PREFIX . "url_alias WHERE `query` = '" . $this->db->escape('mz_page_id=' . (int)$value) . "'");
					} else {
					...
					foreach ($categories as $category) {
						if(version_compare(VERSION, '3.0.0.0') < 0){ // For opencart 2
							$query = $this->db->query("SELECT * FROM " . DB_PREFIX . "url_alias WHERE `query` = 'mz_blog_category_id=" . (int)$category . "'");
						} else {
		...
		if($this->config->get('maza_query_keyword') && version_compare(VERSION, '3.0.0.0') >= 0 && version_compare(VERSION, '4.0.0.0') < 0 && !$url && isset($data['route'])){
