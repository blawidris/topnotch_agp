...
class ControllerExtensionMazalayoutBuilder extends Controller {
                ...
                foreach($shortcode_data as $type => $type_values){
                    ...
                    if($type == 'module'){
                        foreach($type_values as $module){
                            ...
                            $cache_key = $this->mz_theme_config->get('theme_code') . '.' . $this->mz_skin_config->get('skin_code') . '.' . $this->mz_skin_config->get('skin_id') . '.' . $this->config->get('config_store_id') . '.module.layout_builder.' . $module['code'] . '.e' . $module['entry_id'] . $module['suffix'] . '-l' . $this->config->get('config_language_id') . $this->session->data['currency'];
                    ...
                    if($type == 'widget'){
                        foreach($type_values as $widget){
                            ...
                            $cache_key = $this->mz_theme_config->get('theme_code') . '.' . $this->mz_skin_config->get('skin_code') . '.' . $this->mz_skin_config->get('skin_id') . '.' . $this->config->get('config_store_id') . '.widget.layout_builder.' . $widget['code'] . '.e' . $widget['entry_id'] . '-l' . $this->config->get('config_language_id') . $this->session->data['currency'];
                            ...
                            if(empty($data[$shortcode])){
                                ...
                                // $widget['data']['entry_id'] = $widget['entry_id'];
        ...
        private function getUtilityClasses(array $entry): string {
                ...
                // Display
                // $prev_breakpoint_display = 'default';
                // if(!empty($entry['setting'][$breakpoint][$entry['type'] . '_display']) && !in_array($entry['setting'][$breakpoint][$entry['type'] . '_display'], array('default', $prev_breakpoint_display))){
                //     $class[] = 'd-' . (($breakpoint !== 'xs')?$breakpoint . '-' : '') . $entry['setting'][$breakpoint][$entry['type'] . '_display'];
                //     $prev_breakpoint_display = $entry['setting'][$breakpoint][$entry['type'] . '_display'];
                // }
                ...
                foreach($screen_sizes as $screen_size){
                    ...
                    if(isset($entry['setting'][$screen_size])){
                        ...
                        foreach ($entry['setting'][$screen_size][$entry['type'] . '_background_image'] as $layer => $background) {
                            ...
                            if($background['status'] == 'image' && $image){
        ...
        private function _asset_restore($captured_data){
            ...
            foreach($captured_data as $key => $asset){
                ...
                if(isset($asset['document'])){
                    ...
                    foreach($asset['document']->getStyles() as $style){
                        $this->document->addStyle($style['href'], $style['rel'], $style['media']);
                    }
                    foreach($asset['document']->getScripts() as $href){
