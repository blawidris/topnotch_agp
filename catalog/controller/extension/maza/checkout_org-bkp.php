...
class ControllerExtensionMazaCheckout extends Controller {
	...
	public function save(): void {
		...
		if (!$json) {
			...
			if (!$json) {
				// Guest
				if (!$this->customer->isLogged() && $this->request->post['account'] == 'guest') {
					...
					if ($shipping_required) {
						...
						if (!empty($this->request->post['shipping_address_same'])) {
							$this->session->data['guest']['shipping_address'] = $this->request->post['shipping_address_same'];
						} else {
					...
					if ($customer_group_info && !$customer_group_info['approval']) {
						...
						if ($shipping_required) {
							if (!empty($this->request->post['shipping_address_same'])) {
								...
								$address_id = $this->model_account_address->addAddress($customer_id, $this->request->post['shipping']);
