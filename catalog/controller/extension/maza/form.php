...
class ControllerExtensionMazaForm extends Controller {
    public function submit(): void {
        ...
        if($form_info && $this->request->server['REQUEST_METHOD'] == 'POST') {
            ...
            // If validation success then record data and send mail
            if(!$json){
                if($form_info['record']){
                ...
                // Send mail to admin
                if($form_info['mail_admin_status']){
                ...
                if($form_info['mail_customer_status'] && $customer_email){
                    $this->model_extension_maza_common->sendMail($customer_email, $form_info['mail_customer_subject'], $form_info['mail_customer_message']);
                }
