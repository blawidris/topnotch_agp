...
class ControllerExtensionMazaMenu extends Controller {
         ...
         * @return String
         */
        private function getItems($menu_id, $parent_item_id = 0, $top = false){
         ...
         * @return String
         */
        private function getItem($item_info, $top = false){
                ...
                if($item_info['type'] == 'category'){
                    $data['link'] = $this->url->link('product/category', 'path=' . $item_info['setting']['category_top_id']);

                    // Categories
                    $data['categories'] = $this->getCategories($item_info['setting']['category_top_id'], $item_info['setting']['category_depth']);
                ...
                if($item_info['type'] == 'system'){
                    ...
                    if($item_info['setting']['system_item'] == 'language'){
                        ...
                        if(count($results) <= 1) return; // Minimum 2 languages required to display language option
                    ...
                    if($item_info['setting']['system_item'] == 'currency'){
                        ...
                        if(count($results) <= 1) return; // Minimum 2 currencies required to display currencies option
                        foreach ($results as $result) {
                                if ($result['status']) {
         ...
         * @return Array
         */
        private function getCategories($parent_id = 0, $depth = 0, $path = null){
                $category_data = array();
                ...
                foreach($categories as $category){
                        $category_data[] = array(
                            ...
                            'name'          => $category['name'],
                            'children'      => $this->getCategories($category['category_id'], $depth - 1, $path),
         ...
         */
        private function addCSS($item_info) {
