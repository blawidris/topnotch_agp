...
$_['name']     = 'Font Awesome 5 Free';
$_['status']    = TRUE;

// Font css files
if(maza\Registry::config('maza_cdn')){
    $_['css_file'] = 'https://use.fontawesome.com/releases/v5.12.0/css/all.css';
} else {
    $_['css_file'] = 'catalog/view/theme/' . maza\Registry::theme('theme_code') . '/asset/font_icon/fontawesome-free/css/all.css';
}
...
if(!file_exists($font_manager_css_file)){
    ...
    $mz_scss->setFormatter('ScssPhp\ScssPhp\Formatter\Compressed');

    file_put_contents($font_manager_css_file, $mz_scss->compile($scss, FALSE));
}