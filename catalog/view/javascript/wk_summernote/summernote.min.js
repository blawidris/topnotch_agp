...
return aResult};var unique=function(array){var results=[];for(var idx=0,len=array.length;idx<len;idx ++){if(results.indexOf(array[idx])===-1){results.push(array[idx])}}
return results};return{head:head,last:last,initial:initial,tail:tail,prev:prev,next:next,contains:contains,all:all,sum:sum,from:from,clusterBy:clusterBy,compact:compact,unique:unique}})();var NBSP_CHAR=String.fromCharCode(160);var ZERO_WIDTH_NBSP_CHAR='\ufeff';var dom=(function(){var isEditable=function(node){return node&&$(node).hasClass('note-editable')};var isControlSizing=function(node){return node&&$(node).hasClass('note-control-sizing')};var buildLayoutInfo=function($editor){var makeFinder;if($editor.hasClass('note-air-editor')){var id=list.last($editor.attr('id').split('-'));makeFinder=function(sIdPrefix){return function(){return $(sIdPrefix+id)}};return{editor:function(){return $editor},editable:function(){return $editor},popover:makeFinder('#note-popover-'),handle:makeFinder('#note-handle-'),dialog:makeFinder('#note-dialog-')}}else{makeFinder=function(sClassName){return function(){return $editor.find(sClassName)}};return{editor:function(){return $editor},dropzone:makeFinder('.note-dropzone'),toolbar:makeFinder('.note-toolbar'),editable:makeFinder('.note-editable'),codable:makeFinder('.note-codable'),statusbar:makeFinder('.note-statusbar'),popover:makeFinder('.note-popover'),handle:makeFinder('.note-handle'),dialog:makeFinder('.note-dialog')}}};var makePredByNodeName=function(sNodeName){sNodeName=sNodeName.toUpperCase();return function(node){return node&&node.nodeName.toUpperCase()===sNodeName}};var isText=function(node){return node&&node.nodeType===3};var isVoid=function(node){return node&&/^BR|^IMG|^HR/.test(node.nodeName.toUpperCase())};var isPara=function(node){if(isEditable(node)){return!1}
return node&&/^DIV|^P|^LI|^H[1-7]/.test(node.nodeName.toUpperCase())};var isInline=function(node){return!isBodyContainer(node)&&!isList(node)&&!isPara(node)};var isList=function(node){return node&&/^UL|^OL/.test(node.nodeName.toUpperCase())};var isCell=function(node){return node&&/^TD|^TH/.test(node.nodeName.toUpperCase())};var isBlockquote=makePredByNodeName('BLOCKQUOTE');var isBodyContainer=function(node){return isCell(node)||isBlockquote(node)||isEditable(node)};var isAnchor=makePredByNodeName('A');var isParaInline=function(node){return isInline(node)&&!!ancestor(node,isPara)};var isBodyInline=function(node){return isInline(node)&&!ancestor(node,isPara)};var blankHTML=agent.isMSIE?'&nbsp;':'<br>';var nodeLength=function(node){if(isText(node)){return node.nodeValue.length}
return node.childNodes.length};var isEmpty=function(node){var len=nodeLength(node);if(len===0){return!0}else if(!dom.isText(node)&&len===1&&node.innerHTML===blankHTML){return!0}
...
return pred(el)});return ancestors};var commonAncestor=function(nodeA,nodeB){var ancestors=listAncestor(nodeA);for(var n=nodeB;n;n=n.parentNode){if($.inArray(n,ancestors)>-1){return n}}
...
return node};var appendChildNodes=function(node,aChild){$.each(aChild,function(idx,child){node.appendChild(child)});return node};var isLeftEdgePoint=function(point){return point.offset===0};var isRightEdgePoint=function(point){return point.offset===nodeLength(point.node)};var isEdgePoint=function(point){return isLeftEdgePoint(point)||isRightEdgePoint(point)};var isRightEdgeOf=function(node,ancestor){while(node&&node!==ancestor){if(position(node)!==nodeLength(node.parentNode)-1){return!1}
node=node.parentNode}
return!0};var position=function(node){var offset=0;while((node=node.previousSibling)){offset+=1}
...
node=point.node.parentNode;offset=position(point.node)}else if(hasChildren(point.node)){node=point.node.childNodes[offset-1];offset=nodeLength(node)}else{node=point.node;offset=isSkipInnerOffset?0:point.offset-1}
...
return{node:node,offset:offset}};var isSamePoint=function(pointA,pointB){return pointA.node===pointB.node&&pointA.offset===pointB.offset};var isVisiblePoint=function(point){return isText(point.node)||!hasChildren(point.node)||isEmpty(point.node)||!isEdgePoint(point)};var prevPointUntil=function(point,pred){while(point){if(pred(point)){return point}
...
var isSkipOffset=isSkipInnerOffset&&startPoint.node!==point.node;point=nextPoint(point,isSkipOffset)}};var makeOffsetPath=function(ancestor,node){var ancestors=list.initial(listAncestor(node,func.eq(ancestor)));return $.map(ancestors,position).reverse()};var fromOffsetPath=function(ancestor,aOffset){var current=ancestor;for(var i=0,len=aOffset.length;i<len;i++){current=current.childNodes[aOffset[i]]}
return current};var splitNode=function(point){if(isText(point.node)){if(isLeftEdgePoint(point)){return point.node}else if(isRightEdgePoint(point)){return point.node.nextSibling}
return point.node.splitText(point.offset)}
var childNode=point.node.childNodes[point.offset];var clone=insertAfter(point.node.cloneNode(!1),point.node);appendChildNodes(clone,listNext(childNode));paddingBlankHTML(point.node);paddingBlankHTML(clone);return clone};var splitTree=function(root,point){var ancestors=listAncestor(point.node,func.eq(root));if(!ancestors.length){return null}else if(ancestors.length===1){return splitNode(point)}
return ancestors.reduce(function(node,parent){var clone=insertAfter(parent.cloneNode(!1),parent);if(node===point.node){node=splitNode(point)}
appendChildNodes(clone,listNext(node));paddingBlankHTML(parent);paddingBlankHTML(clone);return clone})};var createText=function(text){return document.createTextNode(text)};var remove=function(node,isRemoveChild){if(!node||!node.parentNode){return}
...
dom.remove(node,!1)});$.each(emptyParents,function(idx,node){dom.remove(node,!1)});return new WrappedRange(point.node,point.offset,point.node,point.offset)};var makeIsOn=function(pred){return function(){var ancestor=dom.ancestor(sc,pred);return!!ancestor&&(ancestor===dom.ancestor(ec,pred))}};this.isOnEditable=makeIsOn(dom.isEditable);this.isOnList=makeIsOn(dom.isList);this.isOnAnchor=makeIsOn(dom.isAnchor);this.isOnCell=makeIsOn(dom.isCell);this.isCollapsed=function(){return sc===ec&&so===eo};this.wrapBodyInlineWithPara=function(){if(dom.isEditable(sc)&&!sc.childNodes[so]){return new WrappedRange(sc.appendChild($(dom.emptyPara)[0]),0)}else if(!dom.isInline(sc)||dom.isParaInline(sc)){return this}
var ancestors=dom.listAncestor(sc,func.not(dom.isInline));var topAncestor=list.last(ancestors);if(!dom.isInline(topAncestor)){topAncestor=ancestors[ancestors.length-2]||sc.childNodes[so]}
...
return this};this.insertNode=function(node,isInline){var rng=this.wrapBodyInlineWithPara();var point=rng.getStartPoint();var splitRoot,container,pivot;if(isInline){container=dom.isPara(point.node)?point.node:point.node.parentNode;if(dom.isPara(point.node)){pivot=point.node.childNodes[point.offset]}else{pivot=dom.splitTree(point.node,point)}}else{var ancestors=dom.listAncestor(point.node,dom.isBodyContainer);var topAncestor=list.last(ancestors)||point.node;if(dom.isBodyContainer(topAncestor)){splitRoot=ancestors[ancestors.length-2];container=topAncestor}else{splitRoot=topAncestor;container=splitRoot.parentNode}
pivot=splitRoot&&dom.splitTree(splitRoot,point)}
if(pivot){pivot.parentNode.insertBefore(node,pivot)}else{container.appendChild(node)}
return node};this.toString=function(){var nativeRng=nativeRange();return agent.isW3CRangeSupport?nativeRng.toString():nativeRng.text};this.bookmark=function(editable){return{s:{path:dom.makeOffsetPath(editable,sc),offset:so},e:{path:dom.makeOffsetPath(editable,ec),offset:eo}}};this.getClientRects=function(){var nativeRng=nativeRange();return nativeRng.getClientRects()}};return{create:function(sc,so,ec,eo){if(!arguments.length){if(agent.isW3CRangeSupport){var selection=document.getSelection();if(selection.rangeCount===0){return null}
var nativeRng=selection.getRangeAt(0);sc=nativeRng.startContainer;so=nativeRng.startOffset;ec=nativeRng.endContainer;eo=nativeRng.endOffset}else{var textRange=document.selection.createRange();var textRangeEnd=textRange.duplicate();textRangeEnd.collapse(!1);var textRangeStart=textRange;textRangeStart.collapse(!0);var startPoint=textRangeToPoint(textRangeStart,!0),endPoint=textRangeToPoint(textRangeEnd,!1);sc=startPoint.cont;so=startPoint.offset;ec=endPoint.cont;eo=endPoint.offset}}else if(arguments.length===2){ec=sc;eo=so}
return new WrappedRange(sc,so,ec,eo)},createFromNode:function(node){return this.create(node,0,node,1)},createFromBookmark:function(editable,bookmark){var sc=dom.fromOffsetPath(editable,bookmark.s.path);var so=bookmark.s.offset;var ec=dom.fromOffsetPath(editable,bookmark.e.path);var eo=bookmark.e.offset;return new WrappedRange(sc,so,ec,eo)}}})();var Table=function(){this.tab=function(rng,isShift){var cell=dom.ancestor(rng.commonAncestor(),dom.isCell);var table=dom.ancestor(cell,dom.isTable);var cells=dom.listDescendant(table,dom.isCell);var nextCell=list[isShift?'prev':'next'](cells,cell);if(nextCell){range.create(nextCell,0).select()}};this.createTable=function(colCount,rowCount){var tds=[],tdHTML;for(var idxCol=0;idxCol<colCount;idxCol++){tds.push('<td>'+dom.blank+'</td>')}
tdHTML=tds.join('');var trs=[],trHTML;for(var idxRow=0;idxRow<rowCount;idxRow++){trs.push('<tr>'+tdHTML+'</tr>')}
trHTML=trs.join('');return $('<table class="table table-bordered">'+trHTML+'</table>')[0]}};var Editor=function(){var style=new Style();var table=new Table();this.saveRange=function($editable){$editable.focus();$editable.data('range',range.create())};this.restoreRange=function($editable){var rng=$editable.data('range');if(rng){rng.select();$editable.focus()}};this.currentStyle=function(target){var rng=range.create();return rng?rng.isOnEditable()&&style.current(rng,target):!1};var triggerOnChange=this.triggerOnChange=function($editable){var onChange=$editable.data('callbacks').onChange;if(onChange){onChange($editable.html(),$editable)}};this.undo=function($editable){$editable.data('NoteHistory').undo($editable);triggerOnChange($editable)};this.redo=function($editable){$editable.data('NoteHistory').redo($editable);triggerOnChange($editable)};var recordUndo=this.recordUndo=function($editable){$editable.data('NoteHistory').recordUndo($editable)};var commands=['bold','italic','underline','strikethrough','superscript','subscript','justifyLeft','justifyCenter','justifyRight','justifyFull','insertOrderedList','insertUnorderedList','indent','outdent','formatBlock','removeFormat','backColor','foreColor','insertHorizontalRule','fontName'];for(var idx=0,len=commands.length;idx<len;idx ++){this[commands[idx]]=(function(sCmd){return function($editable,value){recordUndo($editable);document.execCommand(sCmd,!1,value)}})(commands[idx])}
var insertTab=function($editable,rng,tabsize){recordUndo($editable);var tab=dom.createText(new Array(tabsize+1).join(dom.NBSP_CHAR));rng=rng.deleteContents();rng.insertNode(tab,!0);rng=range.create(tab,tabsize);rng.select()};this.tab=function($editable,options){var rng=range.create();if(rng.isCollapsed()&&rng.isOnCell()){table.tab(rng)}else{insertTab($editable,rng,options.tabsize)}};this.untab=function(){var rng=range.create();if(rng.isCollapsed()&&rng.isOnCell()){table.tab(rng,!0)}};this.insertParagraph=function($editable){recordUndo($editable);var rng=range.create();rng=rng.deleteContents();rng=rng.wrapBodyInlineWithPara();var splitRoot=dom.ancestor(rng.sc,dom.isPara);var nextPara=dom.splitTree(splitRoot,rng.getStartPoint());var emptyAnchors=dom.listDescendant(splitRoot,dom.isEmptyAnchor);emptyAnchors=emptyAnchors.concat(dom.listDescendant(nextPara,dom.isEmptyAnchor));$.each(emptyAnchors,function(idx,anchor){dom.remove(anchor)});range.create(nextPara,0).normalize().select();triggerOnChange($editable)};this.insertImage=function($editable,sUrl,filename){async.createImage(sUrl,filename).then(function($image){recordUndo($editable);$image.css({display:'',width:Math.min($editable.width(),$image.width())});range.create().insertNode($image[0]);triggerOnChange($editable)}).fail(function(){var callbacks=$editable.data('callbacks');if(callbacks.onImageUploadError){callbacks.onImageUploadError()}})};this.insertVideo=function($editable,sUrl){recordUndo($editable);var ytRegExp=/^.*(youtu.be\/|v\/|u\/\w\/|embed\/|watch\?v=|\&v=)([^#\&\?]*).*/;var ytMatch=sUrl.match(ytRegExp);var igRegExp=/\/\/instagram.com\/p\/(.[a-zA-Z0-9]*)/;var igMatch=sUrl.match(igRegExp);var vRegExp=/\/\/vine.co\/v\/(.[a-zA-Z0-9]*)/;var vMatch=sUrl.match(vRegExp);var vimRegExp=/\/\/(player.)?vimeo.com\/([a-z]*\/)*([0-9]{6,11})[?]?.*/;var vimMatch=sUrl.match(vimRegExp);var dmRegExp=/.+dailymotion.com\/(video|hub)\/([^_]+)[^#]*(#video=([^_&]+))?/;var dmMatch=sUrl.match(dmRegExp);var youkuRegExp=/\/\/v\.youku\.com\/v_show\/id_(\w+)\.html/;var youkuMatch=sUrl.match(youkuRegExp);var $video;if(ytMatch&&ytMatch[2].length===11){var youtubeId=ytMatch[2];$video=$('<iframe>').attr('src','//www.youtube.com/embed/'+youtubeId).attr('width','640').attr('height','360')}else if(igMatch&&igMatch[0].length){$video=$('<iframe>').attr('src',igMatch[0]+'/embed/').attr('width','612').attr('height','710').attr('scrolling','no').attr('allowtransparency','true')}else if(vMatch&&vMatch[0].length){$video=$('<iframe>').attr('src',vMatch[0]+'/embed/simple').attr('width','600').attr('height','600').attr('class','vine-embed')}else if(vimMatch&&vimMatch[3].length){$video=$('<iframe webkitallowfullscreen mozallowfullscreen allowfullscreen>').attr('src','//player.vimeo.com/video/'+vimMatch[3]).attr('width','640').attr('height','360')}else if(dmMatch&&dmMatch[2].length){$video=$('<iframe>').attr('src','//www.dailymotion.com/embed/video/'+dmMatch[2]).attr('width','640').attr('height','360')}else if(youkuMatch&&youkuMatch[1].length){$video=$('<iframe webkitallowfullscreen mozallowfullscreen allowfullscreen>').attr('height','498').attr('width','510').attr('src','//player.youku.com/embed/'+youkuMatch[1])}else{}
if($video){$video.attr('frameborder',0);range.create().insertNode($video[0]);triggerOnChange($editable)}};this.formatBlock=function($editable,tagName){recordUndo($editable);tagName=agent.isMSIE?'<'+tagName+'>':tagName;document.execCommand('FormatBlock',!1,tagName)};this.formatPara=function($editable){this.formatBlock($editable,'P')};for(var idx=1;idx<=6;idx ++){this['formatH'+idx]=function(idx){return function($editable){this.formatBlock($editable,'H'+idx)}}(idx)};this.fontSize=function($editable,value){recordUndo($editable);document.execCommand('fontSize',!1,3);if(agent.isFF){$editable.find('font[size=3]').removeAttr('size').css('font-size',value+'px')}else{$editable.find('span').filter(function(){return this.style.fontSize==='medium'}).css('font-size',value+'px')}};this.lineHeight=function($editable,value){recordUndo($editable);style.stylePara(range.create(),{lineHeight:value});triggerOnChange($editable)};this.unlink=function($editable){var rng=range.create();if(rng.isOnAnchor()){recordUndo($editable);var anchor=dom.ancestor(rng.sc,dom.isAnchor);rng=range.createFromNode(anchor);rng.select();document.execCommand('unlink')}};this.createLink=function($editable,linkInfo,options){var linkUrl=linkInfo.url;var linkText=linkInfo.text;var isNewWindow=linkInfo.newWindow;var rng=linkInfo.range;recordUndo($editable);if(options.onCreateLink){linkUrl=options.onCreateLink(linkUrl)}
rng=rng.deleteContents();var anchor=rng.insertNode($('<A>'+linkText+'</A>')[0],!0);$(anchor).attr({href:linkUrl,target:isNewWindow?'_blank':''});range.createFromNode(anchor).select();triggerOnChange($editable)};this.getLinkInfo=function($editable){$editable.focus();var rng=range.create().expand(dom.isAnchor);var $anchor=$(list.head(rng.nodes(dom.isAnchor)));return{range:rng,text:rng.toString(),isNewWindow:$anchor.length?$anchor.attr('target')==='_blank':!0,url:$anchor.length?$anchor.attr('href'):''}};this.getVideoInfo=function($editable){$editable.focus();var rng=range.create();if(rng.isOnAnchor()){var anchor=dom.ancestor(rng.sc,dom.isAnchor);rng=range.createFromNode(anchor)}
return{text:rng.toString()}};this.color=function($editable,sObjColor){var oColor=JSON.parse(sObjColor);var foreColor=oColor.foreColor,backColor=oColor.backColor;recordUndo($editable);if(foreColor){document.execCommand('foreColor',!1,foreColor)}
if(backColor){document.execCommand('backColor',!1,backColor)}};this.insertTable=function($editable,sDim){recordUndo($editable);var dimension=sDim.split('x');var rng=range.create();rng=rng.deleteContents();rng.insertNode(table.createTable(dimension[0],dimension[1]));triggerOnChange($editable)};this.floatMe=function($editable,value,$target){recordUndo($editable);$target.css('float',value)};this.resize=function($editable,value,$target){recordUndo($editable);$target.css({width:$editable.width()*value+'px',height:''})};this.resizeTo=function(pos,$target,bKeepRatio){var imageSize;if(bKeepRatio){var newRatio=pos.y/pos.x;var ratio=$target.data('ratio');imageSize={width:ratio>newRatio?pos.x:pos.y/ratio,height:ratio>newRatio?pos.x*ratio:pos.y}}else{imageSize={width:pos.x,height:pos.y}}
$target.css(imageSize)};this.removeMedia=function($editable,value,$target){recordUndo($editable);$target.detach()}};var History=function(){var undoStack=[],redoStack=[];var makeSnapshot=function($editable){var editable=$editable[0];var rng=range.create();return{contents:$editable.html(),bookmark:rng.bookmark(editable)}};var applySnapshot=function($editable,snapshot){$editable.html(snapshot.contents)};this.undo=function($editable){var snapshot=makeSnapshot($editable);if(!undoStack.length){return}
applySnapshot($editable,undoStack.pop());redoStack.push(snapshot)};this.redo=function($editable){var snapshot=makeSnapshot($editable);if(!redoStack.length){return}
applySnapshot($editable,redoStack.pop());undoStack.push(snapshot)};this.recordUndo=function($editable){redoStack=[];undoStack.push(makeSnapshot($editable))}};var Button=function(){this.update=function($container,styleInfo){var checkDropdownMenu=function($btn,value){$btn.find('.dropdown-menu li a').each(function(){var isChecked=($(this).data('value')+'')===(value+'');this.className=isChecked?'checked':''})};var btnState=function(selector,pred){var $btn=$container.find(selector);$btn.toggleClass('active',pred())};var $fontname=$container.find('.note-fontname');if($fontname.length){var selectedFont=styleInfo['font-family'];if(!!selectedFont){selectedFont=list.head(selectedFont.split(','));selectedFont=selectedFont.replace(/\'/g,'');$fontname.find('.note-current-fontname').text(selectedFont);checkDropdownMenu($fontname,selectedFont)}}
...
$editable.html(dom.value($codable)||dom.emptyPara);$editable.height(options.height?$codable.height():'auto');toolbar.activate($toolbar);$editable.focus()}
...
popover.update(layoutInfo.popover(),styleInfo,isAirMode);handle.update(layoutInfo.handle(),styleInfo,isAirMode)},0)};var hScroll=function(event){var layoutInfo=makeLayoutInfo(event.currentTarget||event.target);popover.hide(layoutInfo.popover());handle.hide(layoutInfo.handle())};var hPasteClipboardImage=function(event){var clipboardData=event.originalEvent.clipboardData;if(!clipboardData||!clipboardData.items||!clipboardData.items.length){return}
var layoutInfo=makeLayoutInfo(event.currentTarget||event.target);var item=list.head(clipboardData.items);var isClipboardImage=item.kind==='file'&&item.type.indexOf('image/')!==-1;if(isClipboardImage){insertImages(layoutInfo.editable(),[item.getAsFile()])}};var hHandleMousedown=function(event){if(dom.isControlSizing(event.target)){event.preventDefault();event.stopPropagation();var layoutInfo=makeLayoutInfo(event.target),$handle=layoutInfo.handle(),$popover=layoutInfo.popover(),$editable=layoutInfo.editable(),$editor=layoutInfo.editor();var target=$handle.find('.note-control-selection').data('target'),$target=$(target),posStart=$target.offset(),scrollTop=$document.scrollTop();var isAirMode=$editor.data('options').airMode;$document.on('mousemove',function(event){editor.resizeTo({x:event.clientX-posStart.left,y:event.clientY-(posStart.top-scrollTop)},$target,!event.shiftKey);handle.update($handle,{image:target},isAirMode);popover.update($popover,{image:target},isAirMode)}).one('mouseup',function(){$document.off('mousemove')});if(!$target.data('ratio')){$target.data('ratio',$target.height()/$target.width())}
editor.recordUndo($editable)}};var hToolbarAndPopoverMousedown=function(event){var $btn=$(event.target).closest('[data-event]');if($btn.length){event.preventDefault()}};var hToolbarAndPopoverClick=function(event){var $btn=$(event.target).closest('[data-event]');if($btn.length){var eventName=$btn.attr('data-event'),value=$btn.attr('data-value');var layoutInfo=makeLayoutInfo(event.target);event.preventDefault();var $target;if($.inArray(eventName,['resize','floatMe','removeMedia'])!==-1){var $selection=layoutInfo.handle().find('.note-control-selection');$target=$($selection.data('target'))}
if(editor[eventName]){var $editable=layoutInfo.editable();$editable.trigger('focus');editor[eventName]($editable,value,$target)}else if(commands[eventName]){commands[eventName].call(this,layoutInfo)}
...
hToolbarAndPopoverUpdate(event)}};var EDITABLE_PADDING=24;var hStatusbarMousedown=function(event){event.preventDefault();event.stopPropagation();var $editable=makeLayoutInfo(event.target).editable();var nEditableTop=$editable.offset().top-$document.scrollTop();var layoutInfo=makeLayoutInfo(event.currentTarget||event.target);var options=layoutInfo.editor().data('options');$document.on('mousemove',function(event){var nHeight=event.clientY-(nEditableTop+EDITABLE_PADDING);nHeight=(options.minHeight>0)?Math.max(nHeight,options.minHeight):nHeight;nHeight=(options.maxHeight>0)?Math.min(nHeight,options.maxHeight):nHeight;$editable.height(nHeight)}).one('mouseup',function(){$document.off('mousemove')})};var PX_PER_EM=18;var hDimensionPickerMove=function(event,options){var $picker=$(event.target.parentNode);var $dimensionDisplay=$picker.next();var $catcher=$picker.find('.note-dimension-picker-mousecatcher');var $highlighted=$picker.find('.note-dimension-picker-highlighted');var $unhighlighted=$picker.find('.note-dimension-picker-unhighlighted');var posOffset;if(event.offsetX===undefined){var posCatcher=$(event.target).offset();posOffset={x:event.pageX-posCatcher.left,y:event.pageY-posCatcher.top}}else{posOffset={x:event.offsetX,y:event.offsetY}}
...
$dimensionDisplay.html(dim.c+' x '+dim.r)};var handleDragAndDropEvent=function(layoutInfo,disableDragAndDrop){if(disableDragAndDrop){$document.on('drop',function(e){e.preventDefault()})}else{attachDragAndDropEvent(layoutInfo)}};var attachDragAndDropEvent=function(layoutInfo){var collection=$(),$dropzone=layoutInfo.dropzone,$dropzoneMessage=layoutInfo.dropzone.find('.note-dropzone-message');$document.on('dragenter',function(e){var isCodeview=layoutInfo.editor.hasClass('codeview');if(!isCodeview&&!collection.length){layoutInfo.editor.addClass('dragover');$dropzone.width(layoutInfo.editor.width());$dropzone.height(layoutInfo.editor.height());$dropzoneMessage.text('Drag Image Here')}
collection=collection.add(e.target)}).on('dragleave',function(e){collection=collection.not(e.target);if(!collection.length){layoutInfo.editor.removeClass('dragover')}}).on('drop',function(){collection=$();layoutInfo.editor.removeClass('dragover')});$dropzone.on('dragenter',function(){$dropzone.addClass('hover');$dropzoneMessage.text('Drop Image')}).on('dragleave',function(){$dropzone.removeClass('hover');$dropzoneMessage.text('Drag Image Here')});$dropzone.on('drop',function(event){event.preventDefault();var dataTransfer=event.originalEvent.dataTransfer;if(dataTransfer&&dataTransfer.files){var layoutInfo=makeLayoutInfo(event.currentTarget||event.target);layoutInfo.editable().focus();insertImages(layoutInfo.editable(),dataTransfer.files)}}).on('dragover',!1)};this.bindKeyMap=function(layoutInfo,keyMap){var $editor=layoutInfo.editor;var $editable=layoutInfo.editable;layoutInfo=makeLayoutInfo($editable);$editable.on('keydown',function(event){var aKey=[];if(event.metaKey){aKey.push('CMD')}
...
var eventName=keyMap[aKey.join('+')];if(eventName){event.preventDefault();if(editor[eventName]){editor[eventName]($editable,$editor.data('options'))}else if(commands[eventName]){commands[eventName].call(this,layoutInfo)}}else if(key.isEdit(event.keyCode)){editor.recordUndo($editable)}})};this.attach=function(layoutInfo,options){this.bindKeyMap(layoutInfo,options.keyMap[agent.isMac?'mac':'pc']);layoutInfo.editable.on('mousedown',hMousedown);layoutInfo.editable.on('keyup mouseup',hToolbarAndPopoverUpdate);layoutInfo.editable.on('scroll',hScroll);layoutInfo.editable.on('paste',hPasteClipboardImage);layoutInfo.handle.on('mousedown',hHandleMousedown);layoutInfo.popover.on('click',hToolbarAndPopoverClick);layoutInfo.popover.on('mousedown',hToolbarAndPopoverMousedown);if(!options.airMode){handleDragAndDropEvent(layoutInfo,options.disableDragAndDrop);layoutInfo.toolbar.on('click',hToolbarAndPopoverClick);layoutInfo.toolbar.on('mousedown',hToolbarAndPopoverMousedown);if(!options.disableResizeEditor){layoutInfo.statusbar.on('mousedown',hStatusbarMousedown)}}
var $catcherContainer=options.airMode?layoutInfo.popover:layoutInfo.toolbar;var $catcher=$catcherContainer.find('.note-dimension-picker-mousecatcher');$catcher.css({width:options.insertTableMaxSize.col+'em',height:options.insertTableMaxSize.row+'em'}).on('mousemove',function(event){hDimensionPickerMove(event,options)});layoutInfo.editor.data('options',options);if(options.styleWithSpan&&!agent.isMSIE){setTimeout(function(){document.execCommand('styleWithCSS',0,!0)},0)}
layoutInfo.editable.data('NoteHistory',new History());if(options.onenter){layoutInfo.editable.keypress(function(event){if(event.keyCode===key.ENTER){options.onenter(event)}})}
...
return layoutInfo};this.removeLayout=function($holder,layoutInfo,options){if(options.airMode){$holder.removeClass('note-air-editor note-editable').removeAttr('id contentEditable');layoutInfo.popover.remove();layoutInfo.handle.remove();layoutInfo.dialog.remove()}else{$holder.html(layoutInfo.editable.html());layoutInfo.editor.remove();$holder.show()}}};$.summernote=$.summernote||{};$.extend($.summernote,settings);var renderer=new Renderer();var eventHandler=new EventHandler();$.fn.extend({summernote:function(options){options=$.extend({},$.summernote.options,options);this.each(function(idx,elHolder){var $holder=$(elHolder);renderer.createLayout($holder,options);var info=renderer.layoutInfoFromHolder($holder);eventHandler.attach(info,options);if(dom.isTextarea($holder[0])){$holder.closest('form').submit(function(){$holder.val($holder.code())})}});if(this.first().length&&options.focus){var info=renderer.layoutInfoFromHolder(this.first());info.editable.focus()}
...
this.each(function(i,elHolder){var info=renderer.layoutInfoFromHolder($(elHolder));if(info&&info.editable){info.editable.html(sHTML)}});return this},destroy:function(){this.each(function(idx,elHolder){var $holder=$(elHolder);var info=renderer.layoutInfoFromHolder($holder);if(!info||!info.editable){return}
var options=info.editor.data('options');eventHandler.dettach(info,options);renderer.removeLayout($holder,info,options)});return this}})}))