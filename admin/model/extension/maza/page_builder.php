...
class ModelExtensionMazaPageBuilder extends Model {
	public function addPage($data) {
		$this->db->query("INSERT INTO " . DB_PREFIX . "mz_page SET status = '" . (int)$data['status'] . "', date_added = NOW(), date_modified = NOW()");
	...
	public function editPage($page_id, $data) {
		$this->db->query("UPDATE " . DB_PREFIX . "mz_page SET status = '" . (int)$data['status'] . "', date_modified = NOW() WHERE page_id = '" . (int)$page_id . "'");
	...
	public function getPages($data = array()) {
			$sql = "SELECT * FROM " . DB_PREFIX . "mz_page p LEFT JOIN " . DB_PREFIX . "mz_page_description pd ON (p.page_id = pd.page_id) WHERE pd.language_id = '" . (int)$this->config->get('config_language_id') . "'";
