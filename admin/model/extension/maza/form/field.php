...
class ModelExtensionMazaFormField extends Model {
	public function addField(array $data): int {
		$sql = "INSERT INTO " . DB_PREFIX . "mz_form_field SET form_id = '" . (int)$data['form_id'] . "', status = '" . (int)$data['status'] . "', sort_order = '" . (int)$data['sort_order'] . "', is_required = '" . (int)$data['is_required'] . "', customer = '" . (int)$data['customer'] . "', `type` = '" . $this->db->escape($data['type']) . "', `name` = '" . $this->db->escape($data['name']) . "', `value` = '" . $this->db->escape($data['value']) . "', `validation` = '" . $this->db->escape($data['validation']) . "', `decimal` = '" . ((int)$data['decimal'] > 0 ? (int)$data['decimal'] : 0) . "', `date_added` = NOW(), `date_modified` = NOW()";
