...
class ModelExtensionMazalayoutBuilder extends model {
        ...
        private function addEntry(int $skin_id, string $group, int $group_owner, array $entries, int $parent_entry_id = 0): void {
                foreach ($entries as $entry) {
                    ...
                    // menu
                    // if($entry['type'] == 'menu'){
                    //     $this->db->query("INSERT INTO " . DB_PREFIX . "mz_layout_entry SET `group` = '" . $this->db->escape($group) . "', `group_owner` = '" . (int)$group_owner . "', `type` = 'menu', `code` = '" . $this->db->escape($entry['code']) . "', `setting` = '" . $this->db->escape(json_encode($setting)) . "', parent_entry_id = '" . (int)$parent_entry_id . "', `skin_id` = '" . (int)$skin_id . "'");
                    // }
        ...
        /**
         * Get list of route layout
         * @param Array $data filter and sort info
         * @return Array route layout list
         */
        public function getRouteLayouts(array $data = array()): array {
        ...
        /**
         * Get total of route layout
         * @param Array $data filter and sort info
         * @return Int total
         */
        public function getTotalRouteLayouts(array $data = array()): int {
        ...
        /**
         * Overwrite default skin for a layout
         */
        public function overrideDefaultSkin(int $layout_id, array $data): void {
                $this->db->query("UPDATE `" . DB_PREFIX . "layout` SET mz_override_skin = '" . $this->db->escape(json_encode($data)) . "' WHERE layout_id = '" . (int)$layout_id . "'");
        }
