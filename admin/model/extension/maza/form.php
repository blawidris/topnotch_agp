...
class ModelExtensionMazaForm extends Model {
	public function addForm(array $data): int {
		$this->db->query("INSERT INTO " . DB_PREFIX . "mz_form SET captcha = '" . $this->db->escape($data['captcha']) . "', information_id = '" . (int)$data['information_id'] . "', record = '" . (int)$data['record'] . "', email_field_id = '" . (int)$data['email_field_id'] . "', subject_field_id = '" . (int)$data['subject_field_id'] . "', mail_admin_status = '" . (int)$data['mail_admin_status'] . "', mail_admin_to = '" . $this->db->escape($data['mail_admin_to']) . "', mail_customer_status = '" . (int)$data['mail_customer_status'] . "', date_added = NOW(), date_modified = NOW()");
	...
	public function editForm(int $form_id, array $data): void {
		$this->db->query("UPDATE " . DB_PREFIX . "mz_form SET captcha = '" . $this->db->escape($data['captcha']) . "', information_id = '" . (int)$data['information_id'] . "', record = '" . (int)$data['record'] . "', email_field_id = '" . (int)$data['email_field_id'] . "', subject_field_id = '" . (int)$data['subject_field_id'] . "', mail_admin_status = '" . (int)$data['mail_admin_status'] . "', mail_admin_to = '" . $this->db->escape($data['mail_admin_to']) . "', mail_customer_status = '" . (int)$data['mail_customer_status'] . "', date_modified = NOW() WHERE form_id = '" . (int)$form_id . "'");
