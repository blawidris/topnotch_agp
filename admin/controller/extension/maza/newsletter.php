...
class ControllerExtensionMazaNewsletter extends Controller {
		...
		$this->document->setTitle($this->language->get('heading_title'));
                ...
                $header_data['menu'] = array(
                    ...
                    array('name' => $this->language->get('tab_subscriber'), 'id' => 'tab-mz-subscriber', 'href' => $this->url->link('extension/maza/newsletter/subscriber', 'user_token=' . $this->session->data['user_token'] . $url, true)),
                );
		...
		$this->document->setTitle($this->language->get('heading_title'));
                ...
                $header_data['menu_active'] = 'tab-mz-mail';
                $header_data['buttons'][] = array(
                    'id' => 'button-send',
                    'name' => false,
			...
			if (!$json) {
				...
				switch ($this->request->post['to']) {
					case 'all':
						...
						$email_total = $this->model_extension_maza_newsletter->getTotalSubscribers($filter_data);

						$results = $this->model_extension_maza_newsletter->getSubscribers($filter_data);
					...
					case 'approved':
						...
						foreach ($results as $result) {
							$emails[] = $result['email_id'];
                                            ...
                                            break;
                                        case 'email':
                                                if(!empty($this->request->post['email_id']) && filter_var($this->request->post['email_id'], FILTER_VALIDATE_EMAIL)){
                                                    $email_total = 1;
                                                    $emails[] = $this->request->post['email_id'];
                                                } else {
                                                    $json['error']['custom'] = $this->language->get('error_invalid_email');
                                                }
                                            break;
				}
				...
				if ($emails) {
					...
					$start = ($page - 1) * 10;
					$end = $total_sent = $start + 10;
                                        
                                        if ($end < $email_total) {
						$json['next'] = str_replace('&amp;', '&', $this->url->link('extension/maza/newsletter/send', 'user_token=' . $this->session->data['user_token'] . '&page=' . ($page + 1), true));
					} else {
						$json['next'] = '';
					}
                                        
                                        if($total_sent > $email_total){
                                            $total_sent = $email_total;
                                        }
                                        
                                        if($json['next']){
                                            $json['success'] = sprintf($this->language->get('text_sent'), $total_sent, $email_total);
                                        } else {
