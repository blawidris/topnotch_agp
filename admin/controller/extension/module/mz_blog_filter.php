...
class ControllerExtensionModuleMzBlogFilter extends Controller {
        private $error = array();
        
        public function index() {
                $this->load->language('extension/module/mz_blog_filter');
		...
		$this->document->setTitle($this->language->get('heading_title'));
                ...
                $header_data['buttons'][] = array( // Button save
                    ...
                    'href' => FALSE,
                    'target' => FALSE,
                    'form_target_id' => 'form-mz-blog-filter',
                );
                ...
                if (isset($this->request->get['module_id'])) {
                    $header_data['buttons'][] = array( // Button export
                        ...
                        'icon' => 'fa-download',
                        'class' => 'btn-warning',
                        'href' => $this->url->link('extension/module/mz_blog_filter/export', 'user_token=' . $this->session->data['user_token']. '&module_id=' . $this->request->get['module_id'] . $url, true),
                        'target' => '_self',
                        'form_target_id' => false,
                    );
                    $header_data['buttons'][] = array( // Button delete
                        ...
                        'icon' => 'fa-trash',
                        'class' => 'btn-danger',
                        'href' => $this->url->link('extension/module/mz_blog_filter/delete', 'user_token=' . $this->session->data['user_token'] . '&module_id=' . $this->request->get['module_id'], true),
                        'target' => '_self',
                        'form_target_id' => false,
                    );
                }
                ...
                $header_data['buttons'][] = array(
                    ...
                    'icon' => 'fa-info',
                    'class' => 'btn-default',
                    'href' => 'https://docs.pocotheme.com/#module-maza-blog-filter',
                    'target' => '_blank'
                );
                
                // Form submit id
                $header_data['form_target_id'] = 'form-mz-blog-filter';
                ...
                if($this->request->server['REQUEST_METHOD'] == 'POST' && $this->validateForm()){
                    ...
                    if (!isset($this->request->get['module_id'])) {
                            $module_id = $this->model_extension_maza_module->addModule('mz_blog_filter', $this->mz_skin_config->get('skin_id'), $this->request->post);
                    } else {
                    ...
                    if(isset($module_id)){
                       $this->response->redirect($this->url->link('extension/module/mz_blog_filter', 'user_token=' . $this->session->data['user_token'] . '&module_id=' . $module_id . $url, true)); 
                    }
                }
                ...
                if (!isset($this->request->get['module_id'])) {
			$data['action'] = $this->url->link('extension/module/mz_blog_filter', 'user_token=' . $this->session->data['user_token'] . $url, true);
                        $data['import'] = $this->url->link('extension/module/mz_blog_filter/import', 'user_token=' . $this->session->data['user_token'] . $url, true);
		} else {
			$data['action'] = $this->url->link('extension/module/mz_blog_filter', 'user_token=' . $this->session->data['user_token'] . '&module_id=' . $this->request->get['module_id'] . $url, true);
                        $data['import'] = $this->url->link('extension/module/mz_blog_filter/import', 'user_token=' . $this->session->data['user_token']. '&module_id=' . $this->request->get['module_id'] . $url, true);
		}
		...
		$data['mz_footer'] = $this->load->controller('extension/maza/common/footer');
                $data['mz_column_left'] = $this->load->controller('extension/maza/common/column_left/module', 'mz_blog_filter');
                
		$this->response->setOutput($this->load->view('extension/module/mz_blog_filter', $data));
        }
        
        protected function validateForm() {
		if (!$this->user->hasPermission('modify', 'extension/module/mz_blog_filter')) {
        ...
        public function delete() {
                ...
                if(isset($this->request->get['module_id']) && $this->validateDelete()){
                        ...
                        $this->response->redirect($this->url->link('extension/module/mz_blog_filter', 'user_token=' . $this->session->data['user_token'] . $url, true));
                } else {
                        $this->response->redirect($this->url->link('extension/module/mz_blog_filter', 'user_token=' . $this->session->data['user_token'] . '&module_id=' . $this->request->get['module_id'] . $url, true));
                }
        ...
        public function export(){
                ...
                if($module_setting){
                    ...
                    header('Content-disposition: attachment; filename="module.mz_blog_filter.' . $this->mz_skin_config->get('skin_code') . '(' . $module_setting['name'] . ').json"');
                    
                    echo json_encode(['type' => 'module', 'code' => 'mz_blog_filter', 'setting' => $module_setting]);
                } else {
        ...
        public function import(){
                ...
                if (!$warning) {
                        ...
                        if (is_file($file)) {
                                ...
                                if($data && $data['type'] == 'module' && $data['code'] == 'mz_blog_filter'){
                                    $this->load->model('extension/maza/module');
                                    
                                    if (!$module_id) {
                                            $module_id = $this->model_extension_maza_module->addModule('mz_blog_filter', $this->mz_skin_config->get('skin_id'), $data['setting']);
                                    } else {
