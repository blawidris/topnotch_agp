...
class ControllerExtensionModuleMzBrand extends Controller {
        private $error = array();
        
        public function index() {
                $this->load->language('extension/module/mz_brand');
		...
		$this->document->setTitle($this->language->get('heading_title'));
                ...
                if (isset($this->request->get['module_id'])) {
                    $header_data['buttons'][] = array( // Button export
                        ...
                        'icon' => 'fa-download',
                        'class' => 'btn-warning',
                        'href' => $this->url->link('extension/module/mz_brand/export', 'user_token=' . $this->session->data['user_token']. '&module_id=' . $this->request->get['module_id'] . $url, true),
                        'target' => '_self',
                        'form_target_id' => false,
                    );
                    $header_data['buttons'][] = array( // Button delete
                        ...
                        'icon' => 'fa-trash',
                        'class' => 'btn-danger',
                        'href' => $this->url->link('extension/module/mz_brand/delete', 'user_token=' . $this->session->data['user_token'] . '&module_id=' . $this->request->get['module_id'], true),
                        'target' => '_self',
                        'form_target_id' => false,
                    );
                }
                ...
                $header_data['buttons'][] = array(
                    ...
                    'icon' => 'fa-info',
                    'class' => 'btn-default',
                    'href' => 'https://docs.pocotheme.com/#module-maza-brand',
                    'target' => '_blank'
                );
                
                // Form submit id
                $header_data['form_target_id'] = 'form-mz-brand';
                ...
                if($this->request->server['REQUEST_METHOD'] == 'POST' && $this->validateForm()){
                    ...
                    if (!isset($this->request->get['module_id'])) {
                            $module_id = $this->model_extension_maza_module->addModule('mz_brand', $this->mz_skin_config->get('skin_id'), $this->request->post);
                    } else {
                    ...
                    if(isset($module_id)){
                       $this->response->redirect($this->url->link('extension/module/mz_brand', 'user_token=' . $this->session->data['user_token'] . '&module_id=' . $module_id . $url, true)); 
                    }
                }
                ...
                if (!isset($this->request->get['module_id'])) {
			$data['action'] = $this->url->link('extension/module/mz_brand', 'user_token=' . $this->session->data['user_token'] . $url, true);
                        $data['import'] = $this->url->link('extension/module/mz_brand/import', 'user_token=' . $this->session->data['user_token'] . $url, true);
		} else {
			$data['action'] = $this->url->link('extension/module/mz_brand', 'user_token=' . $this->session->data['user_token'] . '&module_id=' . $this->request->get['module_id'] . $url, true);
                        $data['import'] = $this->url->link('extension/module/mz_brand/import', 'user_token=' . $this->session->data['user_token']. '&module_id=' . $this->request->get['module_id'] . $url, true);
		}
			...
			$global_setting = $this->model_extension_maza_opencart->getModule($this->request->get['module_id']);
                ...
                // Data
                $setting['brand_source']      =   'featured';
                $setting['featured_manufacturer']= array();
                $setting['filter_auto_filter']= 0;
                $setting['filter_category']   =   array();
                $setting['filter_sub_category']=   1;
                $setting['filter_filter']     =   array();
                $setting['filter_sort']       =   'm.sort_order';
                $setting['filter_order']      =   'ASC';
                $setting['filter_limit']      =   10;
                ...
                foreach ($setting['featured_manufacturer'] as $manufacturer_id) {
		...
		foreach ($setting['filter_filter'] as $filter_id) {
			...
			if ($filter_info) {
                ...
                $data['list_sort'] = array(
                    array('id' => 'm.sort_order', 'name' => $this->language->get('text_sort_order')),
                    array('id' => 'm.name', 'name' => $this->language->get('text_name')),
                    array('id' => 'o.total_order', 'name' => $this->language->get('text_sale')),
                    array('id' => 'm.manufacturer_id', 'name' => $this->language->get('text_date_added')),
                );
		...
		$data['mz_footer'] = $this->load->controller('extension/maza/common/footer');
                $data['mz_column_left'] = $this->load->controller('extension/maza/common/column_left/module', 'mz_brand');
                
		$this->response->setOutput($this->load->view('extension/module/mz_brand', $data));
        }
        
        protected function validateForm() {
		if (!$this->user->hasPermission('modify', 'extension/module/mz_brand')) {
		...
		if ($this->request->post['image_height'] <= 0) {
			$this->error['image_height'] = $this->language->get('error_height');
                ...
                if ($this->request->post['filter_limit'] <= 0) {
			$this->error['filter_limit'] = $this->language->get('error_limit');
		}
        ...
        public function delete() {
                ...
                if(isset($this->request->get['module_id']) && $this->validateDelete()){
                        ...
                        $this->response->redirect($this->url->link('extension/module/mz_brand', 'user_token=' . $this->session->data['user_token'] . $url, true));
                } else {
                        $this->response->redirect($this->url->link('extension/module/mz_brand', 'user_token=' . $this->session->data['user_token'] . '&module_id=' . $this->request->get['module_id'] . $url, true));
                }
        ...
        public function export(){
                ...
                if($module_setting){
                    ...
                    header('Content-disposition: attachment; filename="module.mz_brand.' . $this->mz_skin_config->get('skin_code') . '(' . $module_setting['name'] . ').json"');
                    
                    echo json_encode(['type' => 'module', 'code' => 'mz_brand', 'setting' => $module_setting]);
                } else {
        ...
        public function import(){
                ...
                if (!$warning) {
                        ...
                        if (is_file($file)) {
                                ...
                                if($data && $data['type'] == 'module' && $data['code'] == 'mz_brand'){
                                    $this->load->model('extension/maza/module');
                                    
                                    if (!$module_id) {
                                            $module_id = $this->model_extension_maza_module->addModule('mz_brand', $this->mz_skin_config->get('skin_id'), $data['setting']);
                                    } else {
